...
export function loadToken() {
    const token = process.env.REACT_APP_FAKEAPI ? 'Leeloo Dallas Multipass' : localStorageGet(ACCESS_TOKEN_KEY);
    return token;
}
 ...
 */
export function saveRefreshToken(newRefreshToken: string) {
 ...
 */
let _timeout_refresh_token: number = 0;
...
export function setRefreshTimeout(interval = REFRESH_TIMEOUT) {
    ...
    _timeout_refresh_token = window.setTimeout(() => {
...
export function isUserStillLoggedIn() {
    if (process.env.REACT_APP_FAKEAPI) return true; // Bypass for "MultiPass" login emulation mode
...
/**
 * Generates API response alike object to emulate login when env.REACT_APP_FAKEAPI is set
 * @returns {object} response alike object with tokens
 */
export function fakeApiResponse() {
    if (!process.env.REACT_APP_FAKEAPI) return { data: {} }; // The "MultiPass" mode not found
